...
%%
%% @doc Json schema validation module for draft 2020-12.
%%
%% https://json-schema.org/draft/2020-12/json-schema-core.html
%% https://json-schema.org/draft/2020-12/json-schema-validation.html
%%
%% This module is forked from `jesse_validator_draft2019_09'. Draft 2020-12 is
%% identical to 2019-09 except for the array keywords and the dynamic
%% referencing keywords:
%%   * The tuple form of "items" became "prefixItems"; "items" is now the
%%     single-schema applicator for the items *after* any "prefixItems"
%%     (i.e. it replaced 2019-09 "additionalItems"). "additionalItems" is gone.
%%   * "$dynamicRef"/"$dynamicAnchor" replaced "$recursiveRef"/
%%     "$recursiveAnchor". "$dynamicRef" is not yet implemented — see the safety
%%     rule below.
%%
%% Everything else — "$ref" alongside siblings, dependentRequired/
%% dependentSchemas, if/then/else, min/maxContains, $defs/$anchor,
%% annotation-only format, and the unevaluated* annotation model — is inherited
%% unchanged from 2019-09.
%%
...
%% implement (`$dynamicRef') raises `keyword_not_supported' rather than being
...
-module(jesse_validator_draft2020_12).
...
walk( Value
  ...
  walk(Value, Attrs, NewState);
%% Draft 2020-12: the positional/tuple form of "items".
walk(Value, [{?PREFIXITEMS, PrefixItems} | Attrs], State) ->
  NewState = case jesse_lib:is_array(Value) of
               true  -> check_prefix_items(Value, PrefixItems, State);
               false -> State
             end,
  walk(Value, Attrs, NewState);
%% Draft 2020-12: "items" is the single-schema applicator for items *after* any
%% "prefixItems" (it replaced 2019-09 "additionalItems").
...
%% silently ignoring it (which would false-accept). "$dynamicAnchor" is an
%% identifier that carries no assertion, so it falls through to the ignored
%% catch-all below.
walk(Value, [{?DYNAMICREF, _} | Attrs], State) ->
  NewState = unsupported_keyword(?DYNAMICREF, State),
  walk(Value, Attrs, NewState);
...
%% @doc prefixItems (draft 2020-12). Positional/tuple validation of the leading
%% array items; the covered indexes are recorded as evaluated.
%% @private
check_prefix_items(Value, PrefixItems0, State) when is_list(PrefixItems0) ->
  PrefixItems = lists:map(fun canonical/1, PrefixItems0),
...
%% @doc items (draft 2020-12). Single-schema applicator for the items *after*
%% any "prefixItems"; the covered indexes are recorded as evaluated.
%% @private
check_items(Value, Items0, State) ->
  case jesse_lib:is_json_object(Items0) orelse is_boolean(Items0) of
    true ->
      ...
      JsonSchema = get_current_schema(State),
      StartIndex = prefix_items_count(JsonSchema),
      case StartIndex >= length(Value) of
        true ->
          %% No items beyond "prefixItems"; nothing for "items" to do.
          State;
        false ->
          Rest = lists:nthtail(StartIndex, Value),
          {_, TmpState} =
            lists:foldl( fun(Item, {Index, CurrentState}) ->
                             { Index + 1
                             , check_value(Index, Item, Items, CurrentState)
                             }
                         end
                       , {StartIndex, set_current_schema(State, Items)}
                       , Rest
                       ),
          State1 = set_current_schema(TmpState, JsonSchema),
          ev_add_items(State1, lists:seq(StartIndex, length(Value) - 1))
      end;
    _ ->
      handle_schema_invalid({?wrong_type_items, Items0}, State)
  end.

%% @doc Number of leading items covered by "prefixItems" of the current schema.
%% @private
prefix_items_count(JsonSchema) ->
  case schema_keyword(?PREFIXITEMS, JsonSchema) of
    PrefixList when is_list(PrefixList) -> length(PrefixList);
    _                                   -> 0
  end.
