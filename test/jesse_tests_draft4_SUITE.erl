...
%%
%% @doc Draft 04 conformance against the official JSON-Schema-Test-Suite.
...
-define(META, <<"http://json-schema.org/draft-04/schema#">>).
...
%% @doc Cases jesse does not (yet) support for draft 04. `{File, '_'}' skips a
%% whole file; `{File, Description}' skips one case.
skip_list() ->
  [ %% in-document "id"/base-URI resolution: location-independent identifiers,
    %% nearest-parent id resolution, a sibling id changing the base URI.
